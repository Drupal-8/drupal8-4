...
?>>
    ...
    <div class="default-content">
        <div class="date-author-tag">
            <span class="created"><?php echo date("j F Y", $node->created); ?></span>
            <span class="dash">-</span>
            <span class="name"><?php
                $userdata = user_load($node->uid);
                echo $userdata->field_first_name['und'][0]['value'] . " " . $userdata->field_last_name['und'][0]['value'];
            ...
            <span class="tags"><?php
                if (!empty($node->field_tags['und'])) {
                    foreach ($node->field_tags['und'] as $tags) {
                        $tagdata[] = l($tags['taxonomy_term']->name, "blogs/" . $tags['tid']);
                    }
        ...
        <div class="blog-image">
            <?php
            // echo render($content['field_case_image']);
            echo theme("image_style", array('style_name' => 'blog_overview', 'path' => $content['field_case_image']['#items'][0]['uri'], 'attributes' => array('class' => 'avatar')));
            ?>
        </div>
        <?php print $node->body['und'][0]['value']; ?>

        <div class="profile-blog-section">
            <div class="profile-img">
                <?php
                global $base_url;
                if ($userdata->picture->uri) {
                    $image_path = file_create_url($userdata->picture->uri);
                    echo '<img alt="" src="' . $image_path . '"height="94" width="94" alt="user-images" />';
                } else {
                    echo '<img src="' . $base_url . '/sites/default/files/default-avatar.png" height="94" width="94" alt="user-images"/>';
                }
                ?>
                <a class="author-name" href="<?php echo $base_url . "/user-blogs/" . $userdata->uid; ?>"><?php echo $userdata->field_first_name['und'][0]['value'] . " " . $userdata->field_last_name['und'][0]['value']; ?></a>
                <span><?php echo $userdata->field_function_title['und'][0]['value']; ?></span>
            </div>

            <div class="profile-content">
                <?php echo substr($userdata->field_user_description['und'][0]['value'], 0, 227); ?>
            </div>
        </div>
